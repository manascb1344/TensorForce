...
class Stocks extends Component {
	percentChange = stock => {
		return (
			(stock.current_price - stock.avg_entry_price) /
	...
	displayPositions() {
		...
		return stocks.map((stock, index) => (
			<div key={index}>
				<li
					style={this.props.positionsStyle}
					className={this.chooseColor(stock)}
				>
					{stock.symbol}
				</li>
				<p style={this.props.dataStyle}>
					side: {stock.side}, current price: ${stock.current_price}, pct change:{' '}
					{this.percentChange(stock)}%
				</p>
			</div>
		));
	}
