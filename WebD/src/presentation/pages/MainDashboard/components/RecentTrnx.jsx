...
const RecentTrnx = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const apiUrl = 'https://paper-api.alpaca.markets/v2/account/activities';
						...
						({ id, symbol, side, qty, pcColor, price }) => (
							<div key={id} className='flex justify-between mt-4'>
								<div className='flex gap-4'>
									...
									<div>
										<p className='text-md font-semibold'>Order: {side}</p>
										<p className='text-sm text-gray-400'>Quantity: {qty}</p>
									</div>
								</div>
